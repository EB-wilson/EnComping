...
组合类型，即若干个组件组合在一起构造的一个**承担实际功能**的类型，为表述方便简称为组合，其组成结构与组件基本类似：
- 组合的特性修饰符
- 组合名称
- 组合所包含的组件表
- 组合内容

不同的是，在组合当中**不能声明任何不来自其组件的可见功能**，即在组合中不能声明任何从类型外部可以访问的变量和函数等，例如以下声明因为定义了一个可见变量`z`，所以是**非法的**：
...
如果您此前使用过java或者其他传统的面向对象语言，那段声明的实际效果这会有一些反直觉。组合`Entity`事实上同时包含了`Motion`，`Health`还有`Position`，即`Position`与`Motion`是平级的，正如前文提到的在Motion中调用的Position的函数，事实上是在Motion中委派给Position的，而非调用虚方法。
